...
if ($route == "dashboard") {
	if($isAdmin) {
		...
		$sumAssetsAllocate = countTable("assetsallocate");
		$sumAssetsReturn = countTable("assetsreturn");
		$sumConsumables = countTable("consumables");
		$sumLicenses = countTable("licenses");
		$sumLicensesAllocation = countTable("licensesallocate");
                $sumLicensesReturn = countTable("licensereturn");
		...
		$sumAssetsAllocate = countTableFiltered("assetsallocate","clientid",$liu['clientid']);
                $sumAssetsReturn = countTable("assetsreturn");
		$sumConsumables = countTableFiltered("consumables","clientid",$liu['clientid']);
		$sumLicenses = countTableFiltered("licenses","clientid",$liu['clientid']);
		$sumLicensesAllocation = countTableFiltered("licensesallocate","clientid",$liu['clientid']);
		$sumLicensesReturn = countTable("licensereturn");
...
if ($route == "inventory/assets/createcomponents") {
        isAuthorized("addComponents");
	...
	$companies = getTable("company");
	$categories = getTable("assetcategories");
	$models = getTable("models");
	$manufacturers = getTable("manufacturers");
	$correncies = getTable("currency");
	$admins = getTableFiltered("people","type","admin");
	$labels = getTable("labels");
...
//	$customfields = getTable("assets_customfields");
	...
	$pageTitle = __("Create Components");

	}    
        
   if ($route == "inventory/assets/managecomponents") {
	isAuthorized("manageComponents");
	$managecomponents = getRowById("components",$_GET['id']);
...
if ($route == "inventory/assetsreturn") {
	isAuthorized("viewAssetsreturn");
	if($isAdmin) {
		$assetsreturns = getTable("assetsreturn");
                $assets = getTable("assets");
                $categories = getTable("assetcategories");
                $admins = getTableFiltered("people","type","user");
                $departmentcategories = getTable("departmentcategories");
                $labels = getTable("labels");
                $projects = getTable("projects");
                $managers = getTable("managercategories");
                $approvals = getTable("approvalcategories");
	}
	else {
		$assets = getTableFiltered("assetsreturn","clientid",$liu['clientid']);
	}
	$pageTitle = __("Assets Return");
}

if ($route == "inventory/assets/createassetsreturn") {
        isAuthorized("addAssetsreturn");
	...
	$approvals = getTable("approvalcategories");
        //print_r($companies);die();
	$pageTitle = __("Create Assets Return");

	}    
        
   if ($route == "inventory/assets/manageassetsreturn") {
	isAuthorized("manageAssetsreturn");
	$assetsreturns = getRowById("assetsreturn",$_GET['id']);
	isOwner($asset['clientid']);
	$customdata = unserialize($asset['customfields']);
	...
	$pageTitle = 'Manage Assetsretrun';
	} 
// Assets Return end by pratik  
        
       

        
// CONSUMABLES BY PRATIK
if ($route == "inventory/consumables") {
	isAuthorized("viewConsumables");
	if($isAdmin) {
		$consumables = getTable("consumables");
	}
	else {
		$assets = getTableFiltered("consumables","clientid",$liu['clientid']);
	}
	$pageTitle = __("Consumables");
}

if ($route == "inventory/assets/createconsumables") {
        isAuthorized("addConsumables");
...
//	$customfields = getTable("assets_customfields");
	...
	$pageTitle = __("Create Consumables");

	}    
        
   if ($route == "inventory/assets/manageconsumables") {
	isAuthorized("manageConsumables");
	$manageconsumables = getRowById("consumables",$_GET['id']);
...
// Assets_allocate start by pratik
        
if ($route == "inventory/assetsallocate") {
	isAuthorized("viewAssetsallocate");
	if($isAdmin) {
                $assetsAllocates = getTable("assetsallocate");
                $users = getTableFiltered("people","type","user");
                $manufacturers = getTable("manufacturers");
                $consumables = getTable("assets_allocate");
	}
	else {
		$assetsallocate = getTableFiltered("assetsallocate","clientid",$liu['clientid']);
                $assetsAllocates = getTable("assetsallocate");
	}
	$pageTitle = __("Assets Allocate");
}

if ($route == "inventory/assets/createassetsallocate") {
        isAuthorized("addAssetsallocate");

	if($isAdmin) {
                $assetsAllocate = getTable("assetsallocate");
                $users = getTableFiltered("people","type","user");
	}
	else {
		$locations = getTableFiltered("locations","clientid",$liu['clientid']);
	...
	$pageTitle = __("Create Assets Allocate");

	}    
        
   if ($route == "inventory/assets/manageassetsallocate") {
	isAuthorized("manageAssetsallocate");
	$manageAssetsallocate = getRowById("assetsallocate",$_GET['id']);
        isOwner($asset['clientid']);
	...
	$pageTitle = 'Manage Assets Allocate';
	}         
...
if ($route == "sam/licensesallocate") {
	isAuthorized("viewLicensesallocate");
	if($isAdmin) {
            $licensesAllocates = getTable("licensesallocate");
            $users = getTableFiltered("people","type","user");
            $manufacturers = getTable("manufacturers");
	}
	else {
            $licensesallocate = getTableFiltered("licensesallocate","clientid",$liu['clientid']);
	}
	$pageTitle = __("Licenses Allocate");
}

if ($route == "sam/assets/createlicensesallocate") {
        isAuthorized("addLicensesallocate");

	if($isAdmin) {
                $licensesAllocate = getTable("lincesesallocate");
                $users = getTableFiltered("people","type","user");
	}
	else {
		$locations = getTableFiltered("locations","clientid",$liu['clientid']);
	...
	$manufacturers = getTable("manufacturers");
	$admins = getTableFiltered("people","type","admin");
	$pageTitle = __("Create Licenses Allocate");

	}    
        
   if ($route == "sam/assets/managelicensesallocate") {
	isAuthorized("manageLicensesallocate");
	$manageLicensesallocate = getRowById("licensesallocate",$_GET['id']);
        isOwner($asset['clientid']);
	...
	$pageTitle = 'Manage Licenses Allocate';
	}         
        
// Licenses allocate end by pratik end              
        
// License RETURN BY PRATIK
if ($route == "sam/licensereturn") {
	isAuthorized("viewLicensereturn");
	if($isAdmin) {
                $licenses = getTable("licenses");
		$licensereturns = getTable("licensereturn");
                $categories = getTable("licensecategories");
                $admins = getTableFiltered("people","type","user");
                $departmentcategories = getTable("departmentcategories");
                $labels = getTable("labels");
		...
		$assets = getTableFiltered("licensereturn","clientid",$liu['clientid']);
	}
	$pageTitle = __("License Return");
}

if ($route == "sam/licenses/createlicensereturn") {
        isAuthorized("addLicensereturn");
	...
	$approvals = getTable("approvalcategories");
        //print_r($companies);die();
	$pageTitle = __("Create License Return");

	}    
        
   if ($route == "sam/licenses/managelicensereturn") {
//        print_r($licensereturns);die();
	isOwner($asset['clientid']);
	isAuthorized("manageLicensereturn");
	...
	$licensereturns = getRowById("licensereturn",$_GET['id']);
        $licenses = getTable("licenses");
        $licensereturnss = getTable("licensereturn");
        $categories = getTable("licensecategories");
	...
	$pageTitle = 'Manage License Rretrun';
	} 
// License Return end by pratik           
...
// SAM LICENSE CATEGORIES
if ($route == "sam/attributes/licensecategories") { isAuthorized("manageData"); $categories = getTable("licensecategories"); $pageTitle = __("License Categories"); }

// MASTER DEPARTMENT CATEGORIES
if ($route == "master/department/departmentcategories") { isAuthorized("manageMaster"); $departmentsCategories = getTable("departmentcategories"); $pageTitle = __("Department Categories"); }
//Master Department
    if ($route == "master/department/departmentcategories") {
	isAuthorized("viewMaster");
	$departmentsCategories = getTable("departmentcategories");
	$pageTitle = __("Department");
    }

// MASTER DEPARTMENT CATEGORIES
if ($route == "master/manager/managercategories") { isAuthorized("manageMaster"); $categories = getTable("managercategories"); $pageTitle = __("Manager Categories"); }
//Master Department
    if ($route == "master/manager/managercategories") {
	isAuthorized("viewMaster");
	$categories = getTable("managercategories");
	$pageTitle = __("Manager");
    }
    
// MASTER APPROVAL CATEGORIES
if ($route == "master/approval/approvalcategories") { isAuthorized("manageMaster"); $categories = getTable("approvalcategories"); $pageTitle = __("Approval Categories"); }
//Master Department
    if ($route == "master/approval/approvalcategories") {
	isAuthorized("viewMaster");
	$categories = getTable("approvalcategories");
	$pageTitle = __("Approval");
    }
