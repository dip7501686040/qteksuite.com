...
//    global $database;
//    return $table = 'SELECT  assc.name,ass.id as assetid, SELECT (COUNT(assc.name) FROM assetcategories assc WHERE assc.id = ass.categoryid) FROM assets ass';
//    
//}

function abc(){
    global $database;
    echo $table = $database->select();
    //return $tables = 'SELECT assets.purchase_date AS DateOfJoin, assetcategories.name AS catname, assets.tag, people.name AS allocatedBy, labels.name AS actions FROM assets LEFT JOIN assetcategories ON assetcategories.id = assets.categoryid LEFT JOIN people ON people.id = assets.userid LEFT JOIN labels ON labels.id = assets.statusid GROUP BY assets.id ORDER BY assets.id DESC';
}
