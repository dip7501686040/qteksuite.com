...
class Consumables extends App {
    ...
    public static function add($data) {
        ...
        if(isset($data['category'])) {
            ...
            $target_dir = $scriptpath . "uploads/consumables/";
    	...
    	$lastid = $database->insert("consumables", [
    	...
    	if ($lastid == "0") { return "11"; } else { logSystem("Consumables Added - ID: " . $lastid); return "10"; }
    }
    ...
    public static function edit($data) {
        ...
        $statusid = 0;
        $target_file_old = 0; 
        $target_file_old = $database->get("consumables", "image", [ "id" => $data['id'] ]);
        ...
        if(isset($data['currency'])) {
            ...
            if(!empty($_FILES["image"]["name"])){
                ...
                // if everything is ok, try to upload file
                    ...
                    if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
                        echo "The file ". basename( $_FILES["image"]["name"]). " has been uploaded.";
                        $target_file = "/uploads/consumables/".$_FILES["image"]["name"];
                    } else {
                        echo "Sorry, there was an error uploading your file.";
                    }
                }
    	...
    	logSystem("Consumables Edited - ID: " . $data['id']);
    	return "20";
    }
    ...
    public static function delete($id) {
    	global $database;
        $database->delete("consumables", [ "id" => $id ]);
    	logSystem("Consumables Deleted - ID: " . $id);
    	return "30";
    }

    public static function nextConsumablesTag() {
    	global $database;
        $max = $database->max("consumables", "id");
    	return $max+1;
    	}
