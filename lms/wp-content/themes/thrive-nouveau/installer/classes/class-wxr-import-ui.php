...
class WXR_Import_UI {
	...
	public function show_updates_in_header() {
		...
		$message = sprintf(
			esc_html__( 'A new version of this importer is available. Please update to version %s to ensure compatibility with newer export files.', 'thrive-nouveau' ),
		...
		$link = sprintf( '<a href="%s" class="button">%s</a>', $url, esc_html__( 'Update Now', 'thrive-nouveau' ) );
	...
	protected function get_url( $step = 0 ) {
		$path = 'themes.php?page=dsc-demo-installer';
	...
	protected function display_error( WP_Error $err, $step = 0 ) {
		...
		echo '<p><strong>' . __( 'Sorry, there has been an error.', 'thrive-nouveau' ) . '</strong><br />';

		echo self::pass_tf_analysis( $err->get_error_message() );
	...
	public function display_import_progress() {
		$this->display_import_step();
	}

	public function dispatch_import_step() {
		$this->display_author_step();
	}
	...
	protected function render_upload_form() {
		...
		if ( ! empty( $upload_dir['error'] ) ) {
			...
			echo self::pass_tf_analysis( $error );
			return;
		}

		// Queue the JS needed for the page
		$url = get_template_directory_uri() . '/installer/assets/intro.js';
		$deps = array(
			'wp-backbone',
			'wp-plupload',
		);
		...
		$settings = array(
			'l10n' => array(
				'frameTitle' => esc_html__( 'Select', 'thrive-nouveau' ),
				'buttonText' => esc_html__( 'Import', 'thrive-nouveau' ),
			),
			...
			'plupload' => array(
				'filters' => array(
					...
					'mime_types'    => array(
						array(
							'title'      => esc_html__( 'XML files', 'thrive-nouveau' ),
							'extensions' => 'xml',
						),
					),
				),
		...
		$dsc_css_url = get_template_directory_uri() . '/installer/assets/intro.css';

		wp_enqueue_style( 'wxr-import-upload', $dsc_css_url, array(), '20160412' );
	...
	protected function display_author_step() {
		...
		$demo_id = filter_input(INPUT_GET, 'demo_id', FILTER_SANITIZE_STRING);
		
		if ( ! empty( $demo_id ) ) {

			$config = new DemoConfig();

			$demos = $config->get($demo_id);
			...
			esc_html_e('Error: Unable to fetch demo_id from the url', 'thrive-nouveau');
			return;
		}
	...
	protected function handle_upload() {
		...
		if ( isset( $file['error'] ) ) {
			...
			$message = sprintf(
				esc_html__( 'The export file could not be found at %s. It is likely that this was caused by a permissions problem.', 'thrive-nouveau' ),
	...
	public function handle_async_upload() {
		...
		if ( ! current_user_can( 'upload_files' ) ) {
			echo wp_json_encode( array(
				'success' => false,
				'data'    => array(
					'message'  => __( 'You do not have permission to upload files.', 'thrive-nouveau' ),
					'filename' => $filename,
				),
			) );
	...
	protected function handle_select( $id ) {
		if ( ! is_numeric( $id ) || intval( $id ) < 1 ) {
			return new WP_Error(
				...
				__( 'Invalid media item ID.', 'thrive-nouveau' ),
				compact( 'id' )
			);
		}
		...
		if ( ! current_user_can( 'read_post', $attachment->ID ) ) {
			return new WP_Error(
				...
				__( 'You cannot access the selected media item.', 'thrive-nouveau' ),
				compact( 'id', 'attachment' )
			);
		}
	...
	protected function get_data_for_attachment( $id ) {
		...
		$file = get_template_directory() . '/installer/demos/demo-'.absint($id).'/content.xml.txt';

		if ( ! file_exists($file)) {

			return new WP_Error(
				'wxr_importer.file.not_found',
				sprintf(__( 'Error extensible markup language (xml) for content is not found in: %s', 'thrive-nouveau' ), $file),
				compact( 'id' )
			);
		}
		...
		if ( ! update_post_meta( $id, '_wxr_import_info', $data ) ) {
			return new WP_Error(
				...
				__( 'Could not cache information on the import.', 'thrive-nouveau' ),
				compact( 'id' )
			);
		}
	...
	protected function display_import_step() {
		...
		if ( ! isset( $args['import_id'] ) ) {
			// Missing import ID.
			$error = new WP_Error( 'wxr_importer.import.missing_id', __( 'Missing import file ID from request.', 'thrive-nouveau' ) );
	...
	public function stream_import() {
		...
		$file = get_template_directory() . '/installer/demos/demo-'.$this->id.'/content.xml.txt';
	...
	protected function author_select( $index, $author ) {
		esc_html_e( 'Import author:', 'thrive-nouveau' );
		...
		if ( $create_users ) {
			if ( ! $supports_extras ) {
				esc_html_e( 'or create new user with login name:', 'thrive-nouveau' );
				$value = '';
			} else {
				esc_html_e( 'as a new user:', 'thrive-nouveau' );
		...
		if ( ! $create_users && $supports_extras ) {
			esc_html_e( 'assign posts to an existing user:', 'thrive-nouveau' );
		} else {
			esc_html_e( 'or assign posts to an existing user:', 'thrive-nouveau' );
		}

		wp_dropdown_users( array(
			'name' => sprintf( 'user_map[%d]', $index ),
			'multi' => true,
			'show_option_all' => __( '- Select -', 'thrive-nouveau' ),
		));
	...
	public function show_demo_list() {
		include_once get_template_directory() . '/installer/classes/templates/demos.php';
	}

	public function show_export_button() {
		if ( isset($_GET['export']) && 'true' === $_GET['export'] ) {
		?>
			<h3><?php esc_html_e('Export', 'thrive-nouveau'); ?></h3>
			<a href="<?php echo esc_url(admin_url('themes.php?page=dsc-demo-installer&action=export'));?>" class="button button-secondary">
				<?php esc_html_e('Widgets', 'thrive-nouveau'); ?>
			</a>
			<a href="<?php echo esc_url(admin_url('themes.php?page=dsc-demo-installer&action=export-customizer'));?>" class="button button-secondary">
				<?php esc_html_e('Customizer', 'thrive-nouveau'); ?>
			</a>
		<?php
		}
		return;
	...
	public static function pass_tf_analysis( $mixed_data = '' ) {
		if ( ! empty ( $mixed_data ) ) {
			return $mixed_data;
		}
		return '';
