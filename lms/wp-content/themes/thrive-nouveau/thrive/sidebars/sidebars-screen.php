...
<div class="wrap">
	...
	<div class="con-container">
		...
		<h2>
			<?php _e('Sidebars', 'thrive-nouveau'); ?>
		</h2>
		...
		<?php if ($sidebar_has_errors) { ?>
			...
			<?php 
				$sidebar_existing_error_types = array(
					'u' => __('There was an error while adding your sidebar. Sidebar already exists.', 'thrive-nouveau'),
					'c' => __('Invalid sidebar name. Only alphanumeric characters are allowed.', 'thrive-nouveau'),
					'z' => __('There was an error while adding sidebar. Sidebar name is required.', 'thrive-nouveau')
				);
			?>
		...
		<?php
				if ( $sidebar_is_success ) { 	?>
					<div id="message" class="updated below-h2">
						<p><strong><?php _e('Sidebar has been added.','thrive-nouveau'); ?></strong></p>
					</div>
		<?php   } ?>
			...
			<div class="col-wrapper">
				...
				<?php if( !empty( $sidebars ) ){ ?>
					<table class="widefat">
						<thead>
							<tr>
								<th><?php _e( 'Name', 'thrive-nouveau' ); ?></th>
								<th><?php _e( 'Description', 'thrive-nouveau' ); ?></th>       
							</tr>
						</thead>
						<tbody>
						<?php $row = 0; ?>
				...
				<?php }else{ ?>
					<p class="howto"><?php _e( 'No Custom Sidebars Yet.', 'thrive-nouveau' ); ?></p>
				<?php } ?>
			</div>
		</div>
		<div id="col-left">
			<div class="col-wrapper">
				<h3><?php _e( 'Add New Sidebar', 'thrive-nouveau' ); ?></h3>
				<p>
					<?php _e('Use this form to create a new sidebar for the theme (in case you need it!). After you have successfully added a sidebar, you will be able to assign inside a post or page.', 'thrive-nouveau'); ?>
				</p>
				<div class="form-wrap">
					<?php 
					...
					<form method="post" action="admin-ajax.php">
							...
							<div class="form-field">
								<label for="thrive-sidebar-name"><?php _e( 'Name: *', 'thrive-nouveau' ); ?></label>
									<input type="text" id="thrive-sidebar-name" name="thrive-sidebar-name" value="<?php echo esc_attr( $edit__sidebar_name ); ?>" />
									<p class="description"><?php _e( 'Sidebar name (must be unique)', 'thrive-nouveau' ); ?></p>
							</div>
							<div class="form-field">
								<label for="thrive-sidebar-description"><?php _e( 'Description: (optional)', 'thrive-nouveau' ); ?></label>
									...
									<p class="description">
										<?php _e( 'Text description of what/where the sidebar is. Shown on widget management screen.', 'thrive-nouveau' ); ?>
									</p>
							</div>
							...
							<p class="submit">
								<?php if( $is_edit_sidebar && !empty( $edit__sidebar ) ){ ?>
									...
									<input type="submit" class="button button-primary" value="<?php _e( 'Update Sidebar', 'thrive-nouveau' ); ?>">
								<?php } else { ?>
									<input type="hidden" name="action" value="thrive_sidebar_add"/>
									<input type="submit" class="button button-primary" value="<?php _e( 'Add Sidebar', 'thrive-nouveau' ); ?>">
								<?php } ?>
							</p>
