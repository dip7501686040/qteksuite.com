...
function thrive_wp_login_logo() {
    ...
    <style type="text/css">
        ...
        <?php $thrive_logo_background = get_theme_mod('thrive_customizer_header_background_color', '#0288d1'); ?>
...
function thrive_login_stylesheet() {
    ...
    wp_enqueue_style( 'thrive-custom-login', get_template_directory_uri() . '/css/wp-login.css' );
    wp_enqueue_script( 'thrive-custom-login', get_template_directory_uri() . '/js/wp-login.js', array('jquery') );
...
function thrive_login_logo_url_title() {
    ...
    return esc_attr__( 'Go back to main page', 'thrive-nouveau' );
...
function thrive_wp_login_forget_pass() {
    ...
    echo '<div id="thrive-wp-login-lost-pass"><p><a href="'.esc_url(wp_lostpassword_url()).'" title="'.esc_attr('Click to Recover Password','thrive-nouveau').'">'.esc_html__('Lost Password?', 'thrive-nouveau').'</a></p></div>';
}
