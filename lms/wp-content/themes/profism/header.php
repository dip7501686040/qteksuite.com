...
<html <?php language_attributes(); ?>>
  ...
  <?php do_action('profism_before_site');
  
  $menu_type = '';
  if( function_exists(profism_sticky_header) ){
	$menu_type = profism_sticky_header();
  }
  ?>
	
    <header id="masthead"  class="site-header  float-header <?php echo $menu_type ?>" role="banner">
		...
		<?php if( is_front_page() ) : ?>
			<?php
				...
				if ( $banner_type == 'video' && is_front_page() ) :
					profism_banner_video();
				else :
					profism_banner_background();
				endif;
			?>
		<?php else : ?>
			<div class="">
				<div class="header-content other">
					...
					<div class = "breadcrumb" ><?php if (class_exists('WooCommerce')){
							( (is_woocommerce())? woocommerce_breadcrumb() :profism_get_breadcrumb() );
						}else{
							profism_get_breadcrumb();
						}?></div>
					<?php endif; ?>
				</div>
			</div>
